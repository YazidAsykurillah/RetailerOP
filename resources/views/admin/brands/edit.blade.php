...
@section('title', __('general.edit') . ' ' . __('brand.singular'))

@section('content_header')
    <h1>{{ __('general.edit') }} {{ __('brand.singular') }}</h1>
@stop
            ...
            <form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" 
                               value="{{ old('name', $brand->name) }}" placeholder="{{ __('brand.name') }}" required>
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" 
                                  rows="3" placeholder="{{ __('general.description') }}">{{ old('description', $brand->description) }}</textarea>
                        @error('description')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                        ...
                        <div class="input-group">
                            <div class="custom-file">
                                <input type="file" name="logo" id="logo" class="custom-file-input @error('logo') is-invalid @enderror" 
                                       accept="image/*">
                                <label class="custom-file-label" for="logo">{{ __('general.choose_file') ?? 'Choose new file' }}</label>
                            </div>
                        </div>
                        @error('logo')
                ...
                <div class="card-footer">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> {{ __('brand.update') ?? 'Update Brand' }}
                    </button>
